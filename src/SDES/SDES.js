...
let names = [
  ...
  "random_name",
  "super long name",
  "super very long name that has a lot of characters",
  "a",
  "b",
  "123",
  "1",
...
for (let name of names) {
  // let result = hash8(name + secretKey, table);
  let result = crc16(name + secretKey);
  hashedNames.push(result);

  result = (result >>> 0)
    .toString(2) // convert to binary
    .padStart(16, "0") // pad with zeroes
    .slice(-10); // truncate to 10 bits

  if (hashedSet.has(result)) {
...
//   const inBuffer = Buffer.from(action, "utf-8");
//   const encrypted = BufferCrypt.sdesEncrypt(0, 1987);
//   console.log(`${action} -> ${encrypted}`);
// }
